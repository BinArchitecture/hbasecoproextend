...
public class MaxAggResultHandler implements AggResultHandlerInterface{
	
	private static AggResultHandlerInterface instance=new MaxAggResultHandler();
	...
	public void aggResult(List<ScanAggRunable> lls, AggResult result ,String classType) throws IllegalStateException{
		for (ScanAggRunable srn : lls) {
			...
			if (classType.equals(String.class.getSimpleName())) {
				String bb= srn.getResult().getStgResultString();
				String sq= result.getStgResultString();
				result.setStgResultString(sq==null?bb:bb.compareTo(sq)>=0?bb:sq);
			} else if (classType.equals(Double.class.getSimpleName())) {
				BigDecimal bb= srn.getResult().getStgResult();
				BigDecimal sq= result.getStgResult();
				result.setStgResult(sq==null?bb:bb.compareTo(sq)>=0?bb:sq);
			} else if (classType.equals(Date.class.getSimpleName())) {
				Date bb= srn.getResult().getStgResultDate();
				Date sq= result.getStgResultDate();
				result.setStgResultDate(sq==null?bb:bb.compareTo(sq)>=0?bb:sq);
			}
		}
	}
}