...
public class MinAggHandler implements AggHandlerIntegerface{
	private static MinAggHandler instance=new MinAggHandler();
	private MinAggHandler(){
	}
	public static MinAggHandler getInstance(){
		return instance;
	}
	@Override
	public AggResult agg(String tableName, String qulifier, String classType,
		...
		try {
			if (classType.equals(String.class.getSimpleName())) {
				String agg = aggregationClient.min(TableName.valueOf(tableName), qulifier, classType, scan,
						scanRange);
				result.setStgResultString(agg);

			} else if (classType.equals(Double.class.getSimpleName())) {
				BigDecimal agg = aggregationClient.min(TableName.valueOf(tableName), qulifier, classType, scan,
						scanRange);
				result.setStgResult(agg);
			} else if (classType.equals(Date.class.getSimpleName())) {
				Date agg = aggregationClient.min(TableName.valueOf(tableName), qulifier, classType, scan,
						scanRange);
