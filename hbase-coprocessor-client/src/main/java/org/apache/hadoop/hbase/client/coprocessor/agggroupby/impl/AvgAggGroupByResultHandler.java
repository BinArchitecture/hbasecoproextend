...
public class AvgAggGroupByResultHandler implements AggGroupByResultHandlerInterface{
	
	private static AggGroupByResultHandlerInterface instance=new AvgAggGroupByResultHandler();
	...
	public void aggResult(List<GroupByScanRunable> lls, AggGroupResult aggResult,
			...
			List<Avg> lla=new ArrayList<Avg>();
			for(GroupByScanRunable srn:lls){
				AggGroupResult agg=srn.getResult();
				if(agg==null)
					throw new IllegalStateException("avg can not be null");
				lla.addAll(agg.getListAvg());
			}
//			Map<String,Double> stgResult=HbaseUtil.calcAvg(lla);
			aggResult.setListAvg(lla);
	}
}
