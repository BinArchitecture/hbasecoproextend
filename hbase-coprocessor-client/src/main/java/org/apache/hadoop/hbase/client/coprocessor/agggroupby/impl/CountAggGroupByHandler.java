...
public class CountAggGroupByHandler implements AggGroupByHandlerIntegerface{
	private static CountAggGroupByHandler instance=new CountAggGroupByHandler();
	private CountAggGroupByHandler(){
	}
	public static CountAggGroupByHandler getInstance(){
		return instance;
	}
	@Override
	public AggGroupResult groupBy(String tableName,String qulifier,String classType, AggGroupByEnum type,
		...
		try {
			...
			result.setRowCountResult(aggregationGroupByClient.rowCount(TableName.valueOf(tableName), qulifier, groupby, scan,scanRange));
			return result;
		} catch (Throwable e) {
			e.printStackTrace();
		}
