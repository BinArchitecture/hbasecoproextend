...
public class CountAggGroupByResultHandler implements AggGroupByResultHandlerInterface{
	
	private static AggGroupByResultHandlerInterface instance=new CountAggGroupByResultHandler();
	...
	public void aggResult(List<GroupByScanRunable> lls, AggGroupResult aggResult,
		...
		List<Map<String,Integer>> lla=new ArrayList<Map<String,Integer>>(lls.size());
		for(GroupByScanRunable srn:lls){
			AggGroupResult agg=srn.getResult();
			if(agg==null)
				throw new IllegalStateException("count can not be null");
			lla.add(agg.getRowCountResult());
		}
		Map<String,Integer> rowCountResult=HbaseUtil.calcCount(lla);
		aggResult.setRowCountResult(rowCountResult);
	}
}
