...
public class MaxAggGroupByHandler implements AggGroupByHandlerIntegerface{
	private static MaxAggGroupByHandler instance=new MaxAggGroupByHandler();
	private MaxAggGroupByHandler(){
	}
	public static MaxAggGroupByHandler getInstance(){
		return instance;
	}
	@Override
	public AggGroupResult groupBy(String tableName,String qulifier,String classType, AggGroupByEnum type,
		...
		try {
			...
			if(classType.equals(String.class.getSimpleName())){
				Map<String,String> stgResult=aggregationGroupByClient.max(TableName.valueOf(tableName), qulifier,classType, groupby, scan,scanRange);
				result.setStgResultString(stgResult);
			}
			else if(classType.equals(Double.class.getSimpleName())){
				Map<String,Double> stgResult=aggregationGroupByClient.max(TableName.valueOf(tableName), qulifier,classType, groupby, scan,scanRange);
				result.setStgResult(stgResult);
			}
			else if(classType.equals(Date.class.getSimpleName())){
				Map<String,Date> stgResult=aggregationGroupByClient.max(TableName.valueOf(tableName), qulifier,classType, groupby, scan,scanRange);
