...
public class SumAggGroupByHandler implements AggGroupByHandlerIntegerface{
	private static SumAggGroupByHandler instance=new SumAggGroupByHandler();
	private SumAggGroupByHandler(){
	}
	public static SumAggGroupByHandler getInstance(){
		return instance;
	}
	@Override
	public AggGroupResult groupBy(String tableName,String qulifier,String classType, AggGroupByEnum type,
		...
		try {
			...
			Map<String, Double> stgResult=aggregationGroupByClient.sum(TableName.valueOf(tableName), qulifier, groupby, scan,scanRange);
